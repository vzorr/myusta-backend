...
const { createJobSchema, jobIdSchema, jobApplicationsForCustomerSchema, getUstaAppliedJobsSchema } = require('../validators/job.validator');
...
router.get('/:id/applications', authenticate, authorized(ROLES.CUSTOMER),  validate(jobApplicationsForCustomerSchema), jobController.getJobApplications);
...
router.get('/usta/applied-jobs', authenticate, authorized(ROLES.USTA),validate(getUstaAppliedJobsSchema), jobController.getUstaAppliedJobs);
